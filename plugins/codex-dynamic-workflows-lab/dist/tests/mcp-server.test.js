...
test("MCP workflow_submit preserves codex-auth-only policy into the runner", async () => {
    ...
    const submit = response.result?.structuredContent;
...
function callMcp(requests, options = {}) {
...
function callMcpFirstResponse(request) {
    ...
    return new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => {
            if (code === 0)
                ...
                rejectClosed(new Error(`MCP process exited ${code}: ${stderr}`));
        });
...
async function waitForSummary(artifacts, runId) {
