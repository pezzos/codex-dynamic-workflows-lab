...
import { createBenchmarkManifest, postflightBenchmarkArtifacts, preflightBenchmarkTarget, } from "./benchmark.js";
...
async function main() {
    ...
    if (command === "benchmark-preflight") {
        const target = rest[0];
        if (!target)
            throw new Error("benchmark-preflight requires target directory");
    ...
    if (command === "benchmark-manifest") {
        const target = rest[0];
        if (!target)
            throw new Error("benchmark-manifest requires target directory");
        const options = parseBenchmarkOptions(rest.slice(1));
        printJson(createBenchmarkManifest({
    ...
    if (command === "benchmark-artifact-scan") {
        ...
        if (!artifactRoot)
            throw new Error("benchmark-artifact-scan requires artifact root directory");
...
function parseBenchmarkOptions(args) {
    const options = {
        ...
        campaignFamilyId: undefined,
        cohortId: undefined,
        repeatIndex: undefined,
        fixtureId: undefined,
        sanitizedFixtureHash: undefined,
    ...
    for (let index = 0; index < args.length; index++) {
        const arg = args[index];
        if (arg === "--campaign-id")
            options.campaignId = args[++index];
        else if (arg === "--campaign-family-id")
            options.campaignFamilyId = args[++index];
        else if (arg === "--cohort-id")
            options.cohortId = args[++index];
        else if (arg === "--repeat-index")
            options.repeatIndex = Number(args[++index]);
        else if (arg === "--fixture-id")
            options.fixtureId = args[++index];
        else if (arg === "--sanitized-fixture-hash")
            options.sanitizedFixtureHash = args[++index];
        else if (arg === "--target-state-hash")
            options.targetStateHash = args[++index];
        else if (arg === "--preflight-status")
            options.preflightStatus = args[++index];
        else if (arg === "--target-mode")
            options.targetMode = args[++index];
        else if (arg === "--method")
            options.method = args[++index];
        else if (arg === "--profile")
            options.profiles.push(args[++index]);
        else if (arg === "--note")
            options.notes.push(args[++index]);
        else
            throw new Error(`unsupported benchmark option: ${arg}`);
    }
