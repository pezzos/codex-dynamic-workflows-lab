...
async function copyCodexAuthOnly(env, workerCodexHome) {
    ...
    try {
        ...
        if (sourceAuthStat.isSymbolicLink())
            throw new Error("auth.json must not be a symlink");
        if (!sourceAuthStat.isFile())
            throw new Error("auth.json must be a regular file");
...
function extractFinalResult(events) {
    for (let index = events.length - 1; index >= 0; index--) {
        ...
        if (event?.type === "item.completed" && event.item?.type === "agent_message")
            return redactValue(event.item.text);
        if (event?.type === "turn.completed" && event.result !== undefined)
            return redactValue(event.result);
    }
