...
`),
      /forbids worker write requests|unsupported agent option/,
      options,
    );
  }
});
...
test("runWorkflow aggregates agent evidence validity", async () => {
  const dir = await mkdtemp(join(tmpdir(), "codex-flow-test-"));
  const validityRunner: AgentRunner = {
    async run(input) {
      return {
        agentId: input.label,
        label: input.label,
...
test("runWorkflow resolves route profiles without changing legacy agent result shape", async () => {
  const dir = await mkdtemp(join(tmpdir(), "codex-flow-test-"));
  const observed: Array<{ label: string; profile?: string; reasoningEffort?: string }> = [];
  const routeRunner: AgentRunner = {
    async run(input) {
      observed.push({
        label: input.label,
...
`,
  ...
  assert.equal((result.result as any).scout.schemaName, "scout_map");
  assert.ok((result.result as any).scout.byteLength <= 1024);
...
test("runWorkflow redacts runner results in agent and summary artifacts", async () => {
  const dir = await mkdtemp(join(tmpdir(), "codex-flow-test-"));
  const secretRunner: AgentRunner = {
    async run(input) {
      return {
        agentId: input.label,
        label: input.label,
...
test("runWorkflow invalidates benchmark evidence when target git status changes", async () => {
  ...
  const writingRunner: AgentRunner = {
    async run() {
