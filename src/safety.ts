...
const exportablePatterns: Array<[string, RegExp]> = [
...
const repoPathPatterns: Array<{ kind: string; pattern: RegExp; severity: "blocker" | "warning" }> = [
...
export function scanExportableValue(value: unknown): SafetyFinding[] {
  return scanExportableText(JSON.stringify(value));
}

export function scanExportableText(text: string, path?: string): SafetyFinding[] {
  const findings: SafetyFinding[] = [];
  for (const [kind, pattern] of exportablePatterns) {
    if (pattern.test(text)) findings.push({ kind, path, detail: `secret-like ${kind} pattern` });
  }
  return findings;
}

export function assertExportableSafe(value: unknown): void {
...
export async function scanTargetForSecretLike(root: string, options: { maxFiles?: number; maxBytesPerFile?: number } = {}): Promise<SafetyFinding[]> {
  ...
  async function walk(dir: string): Promise<void> {
    ...
    for (const entry of entries) {
      if (visited >= maxFiles) return;
      if (ignoredTargetDirs.has(entry.name)) continue;
      ...
      if (ignoredTargetExtensions.has(fileExtension(entry.name))) continue;
      visited++;
      for (const { kind, pattern, severity } of repoPathPatterns) {
        if (pattern.test(rel)) findings.push({ kind, path: rel, detail: `secret-like file path: ${rel}`, severity });
      }
      const stat = await lstat(path).catch(() => undefined);
      if (!stat || stat.size > maxBytesPerFile) continue;
      const text = await readFile(path, "utf8").catch(() => "");
      findings.push(
        ...scanExportableText(text, rel).map((finding) => ({
          ...finding,
...
export async function scanArtifactRootForSecretLike(
  root: string,
